...
public:
	...
	// NOTE(landracer): exposed for reuse by the wizard-driven
	// FabExporter pipeline (src/svg/fabexporter.cpp), which needs the
	// same per-board per-layer SVG render the legacy batch panelizer
	// uses. Implementation is unchanged and still resides below.
