...
)
{
	...
	for (
		...
		const QString moduleId =
			footprint.moduleId.trimmed();

		if (moduleId.isEmpty()) {
			result.error =
				QString(
					"%1 does not have a resolved Fritzing module ID."
				)
					.arg(footprint.component);
		...
		if (
			footprint.pinConnectorIds.size() !=
			footprint.pinCount
		) {
			result.error =
				QString(
					"%1 resolved %2 physical pin connectors, "
					"but requires %3."
				)
					.arg(footprint.component)
					.arg(footprint.pinConnectorIds.size())
					.arg(footprint.pinCount);

			return result;
		}

		for (int pin = 1; pin <= footprint.pinCount; ++pin) {
			if (
				!footprint.pinConnectorIds.contains(pin) ||
				footprint.pinConnectorIds.value(pin).trimmed().isEmpty()
			) {
				result.error =
					QString(
						"%1 is missing a connector ID for physical pin %2."
					)
						.arg(footprint.component)
						.arg(pin);
		...
		/*
		 * Native parts must already exist in the Fritzing
		 * reference model. Only generic fallback parts may
		 * be generated by PartFactory.
		 */
		if (
			modelPart == nullptr &&
		...
		if (modelPart == nullptr) {
			result.error =
				footprint.nativePart
					? QString(
						"Resolved native Fritzing part "
						"%1 for %2 is unavailable."
					)
						.arg(moduleId)
						.arg(footprint.component)
					: QString(
	...
	for (const PreparedDip &prepared : preparedDips) {
		...
		const QString dipPin1Id =
			footprint.pinConnectorIds.value(
				1
			);
