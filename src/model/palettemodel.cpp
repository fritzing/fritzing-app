...
ModelPart * PaletteModel::loadPart(const QString & path, bool update) {
	...
	if (root.isNull()) {
		//QMessageBox::information(nullptr, QObject::tr("Fritzing"), QObject::tr("The file is not a Fritzing file (8)."));
		return nullptr;
	}

	if (root.tagName() != "module") {
		//QMessageBox::information(nullptr, QObject::tr("Fritzing"), QObject::tr("The file is not a Fritzing file (9)."));
		return nullptr;
	}

	moduleID = root.attribute("moduleId");
	if (moduleID.isNull() || moduleID.isEmpty()) {
		//QMessageBox::information(nullptr, QObject::tr("Fritzing"), QObject::tr("The file is not a Fritzing file (10)."));
		return nullptr;
	}
