...
//
//      more testing: connectors, no powers, no grounds
//
/////////////////////////////////
...
QString BrdApplication::genSchematic(QDomElement & root, QDomElement & paramsRoot, DifParam * difParam) 
{
    ...
    qreal width = (qCeil(textWidth / unitLength) * unitLength) + rectThickness + (pinLength * 2);
	...
	height += qMax(titles.count(), qMax(0, qMax(lefts.count() - leftsBuses - 1, rights.count() - rightsBuses - 1))) * unitLength;
	...
	svg += QString("<rect x='%4' y='%3' fill='none' width='%1' height='%2' stroke='#000000' stroke-linejoin='round' stroke-linecap='round' stroke-width='%5' />\n")
						.arg(width - pinLength - pinLength - (rectThickness / 2))
    ...
    y -= unitLength * qCeil(titles.count() / 2.0);
	...
	qreal lx = width / 2;
	if ((powers.count() - powersBuses) % 2 == 0) lx -= (unitLength / 2);
	...
	lx = width / 2;
	if ((grounds.count() - groundsBuses) % 2 == 0) lx -= (unitLength / 2);
